...
export default {
  ...
  watch: {
    currentLevel() {
      if (this.currentLevel <= this.$root.questions.length) {
        ...
        this.hintElement.innerHTML = md.render(
          `\`\`\`css\n${this.hint}\n\`\`\``
        );
  ...
  mounted() {
    this.currentLevel = 1;
    this.hintElement = this.$el.querySelector(".hint");
  },
  updated() {
    this.preCodeElement = this.$el.querySelector(".hint pre");
    this.preCodeElement.className = "";
    const isNotRender = this.preCodeElement.classList.length === 0;
    isNotRender && Prism.highlightAllUnder(this.preCodeElement);
  },
  methods: {
    update() {
      this.time = 0;
...
<template>
  <div class="question">
    ...
    <div class="author">LenguajeCSS.com</div>
    <time>{{ toHuman(time) }}</time>
  </div>
</template>

<style src="./QuizQuestion.css"></style>
